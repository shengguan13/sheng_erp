<template>
  <div>
    ...
    <a-input-search v-if="kind === 'supplier'||kind==='batch'||kind==='zeroBatch'||kind==='allocation'||kind === 'sn'||kind === 'snAdd'" v-model="names" placeholder="请点开弹窗" readOnly @search="onSearch"></a-input-search>
    ...
    <j-select-zero-batch-modal v-if="kind === 'zeroBatch'" ref="selectModal" :rows="rows" :multi="multi" :bar-code="value" @ok="selectOK" @initComp="initComp"/>
...
<script>
  ...
  import JSelectZeroBatchModal from './modal/JSelectZeroBatchModal'
  ...
  export default {
    name: 'JSelectList',
    components: {JSelectMaterialModal, JSelectAllocationModal, JSelectBatchModal, JSelectZeroBatchModal, JSelectSupplierModal},
    props: {
      value: {
        type: String,
        required: false
