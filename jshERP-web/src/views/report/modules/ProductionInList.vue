    ...
    data () {
      return {
        ...
        columns: [
          ...
          { title: '入库日期', dataIndex: 'operTime', width: 110}
        ],
        labelCol: {
          xs: { span: 1 },
          sm: { span: 2 },
        ...
        url: {
          list: "/depotItem/findProductionInByDepotIdsAndMaterialId"
        }
      }
    },
    created() {
      ...
      myHandleDetail(record) {
        ...
        findBillDetailByNumber({ number: record.number }).then((res) => {
          if (res && res.code === 200) {
            ...
            that.$refs.billDetail.show(res.data, '生产入库');
            that.$refs.billDetail.title="详情";
          }
        })
      },
      exportExcel() {
        let aoa = [['单据编号', '条码', '名称', '仓库名称', '数量', '日期']]
        for (let i = 0; i < this.dataSource.length; i++) {
          let ds = this.dataSource[i]
          let item = [ds.number, ds.barCode, ds.materialName, ds.depotName, ds.basicNumber, ds.operTime]
          aoa.push(item)
        }
        openDownloadDialog(sheet2blob(aoa), '生产入库详情')
      }
    }
  }
</script>
