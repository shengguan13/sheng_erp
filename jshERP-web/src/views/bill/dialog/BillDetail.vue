  ...
  export default {
    ...
    methods: {
      ...
      retailExportExcel() {
        ...
        let title = ['仓库名称', '条码', '名称', '内部零件号', '客户零件号', '颜色', '扩展信息', '库存', '单位', '序列号', '批号', '有效期', '多供应商', '数量', '单价', '金额', '备注']
        aoa.push(title)
      ...
      orderExportExcel() {
        ...
        let title = ['条码', '名称', '内部零件号', '客户零件号', '颜色', '扩展信息', '库存', '单位', '多供应商', '数量', finishType, '单价', '金额', '税率(%)', '税额', '价税合计', '备注']
        aoa.push(title)
        for (let i = 0; i < this.dataSource.length; i++) {
          let ds = this.dataSource[i]
      ...
      purchaseSaleExportExcel() {
        ...
        let title = ['仓库名称', '条码', '名称', '内部零件号', '客户零件号', '颜色', '扩展信息', '库存', '单位', '序列号', '批号', '有效期', '多供应商', '数量', '单价', '金额', '税率(%)', '税额', '价税合计', '重量', '备注']
        aoa.push(title)
        for (let i = 0; i < this.dataSource.length; i++) {
          let ds = this.dataSource[i]
          let item = [ds.depotName, ds.barCode, ds.name, ds.internalId, ds.model, ds.color, ds.materialOther, ds.stock, ds.unit,
      ...
      allocationOutExportExcel() {
        ...
        let title = ['仓库名称', '条码', '名称', '内部零件号', '客户零件号', '颜色', '扩展信息', '库存', '调入仓库', '单位', '多供应商', '数量', '单价', '金额', '备注']
        aoa.push(title)
        for (let i = 0; i < this.dataSource.length; i++) {
          let ds = this.dataSource[i]
      ...
      assembleExportExcel() {
        let aoa = []
        aoa = [['单据日期：', this.model.operTimeStr, '', '单据编号：', this.model.number],[]]
        let title = ['商品类型', '仓库名称', '条码', '名称', '内部零件号', '客户零件号', '颜色', '扩展信息', '库存', '单位', '多供应商', '数量', '单价', '金额', '备注']
        aoa.push(title)
        for (let i = 0; i < this.dataSource.length; i++) {
          let ds = this.dataSource[i]
