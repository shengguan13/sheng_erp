      ...
      return new Promise((resolve, reject) => {
        getAction("/cas/client/validateLogin",userInfo).then(response => {
          ...
          if(response.success){
            ...
            Vue.ls.set(ACCESS_TOKEN, result.token, 7 * 60 * 60 * 1000)
            Vue.ls.set(USER_NAME, userInfo.username, 7 * 60 * 60 * 1000)
            Vue.ls.set(USER_INFO, userInfo, 7 * 60 * 60 * 1000)
        ...
        login(userInfo).then(response => {
          if(response.code ==200){
            if(response.data.msgTip == 'user can login'){
              ...
              Vue.ls.set(USER_ID, result.user.id, 7 * 60 * 60 * 1000);
              Vue.ls.set(USER_LOGIN_NAME, result.user.loginName, 7 * 60 * 60 * 1000);
              Vue.ls.set(ACCESS_TOKEN, result.token, 7 * 60 * 60 * 1000)
              Vue.ls.set(USER_INFO, result.user, 7 * 60 * 60 * 1000)
              commit('SET_TOKEN', result.token)
            }
            commit('SET_INFO', userInfo)
            resolve(response)
