...
export const JeecgListMixin = {
  ...
  methods:{
    ...
    batchSetCheckStatus: function (status) {
      if(!this.url.batchSetCheckStatusUrl){
        this.$message.error("请设置url.batchSetCheckStatusUrl属性!")
        return
      }
      if (this.selectedRowKeys.length <= 0) {
        this.$message.warning('请选择一条记录！');
        ...
        this.$confirm({
          ...
          onOk: function () {
            that.loading = true;
            postAction(that.url.batchSetCheckStatusUrl, {status: status, ids: ids}).then((res) => {
              if(res.code === 200){
                that.loadData();
                that.onClearSelected();
              } else {
    ...
    handleDetail:function(record, type, prefixNo, showPrice){
      this.$refs.modalDetail.show(record, type, prefixNo, showPrice);
    ...
    tableAddTotalRow(columns, dataSource) {
      if(dataSource.length>0 && this.ipagination.pageSize%10===1) {
        ...
        let parseCols = 'initialStock,currentStock,currentStockPrice,initialAmount,thisMonthAmount,currentAmount,inSum,inSumPrice,' +
          ...
          'needDebt,realNeedDebt,finishDebt,debt,sumPrice,sumBack,sumSalePrice,sumBuyPrice'
